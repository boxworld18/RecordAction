...
function getLocalHintsForElement(element) {
    ...
    //     this.checkForAngularJs = checkForAngularJs();
    // }
    if (!isClickable) isClickable = (checkForAngularJs())(element);
    ...
    switch (tagName) {
        ...
        case "label":
            if (!isClickable) {
                ...
                ((getLocalHintsForElement(element.control)).length === 0);
            }
            break;
        case "body":
            if (!isClickable) {
...
function getLocalHints() {
    ...
    for (const element of Array.from(elements)) {
        const hints = getLocalHintsForElement(element);
    ...
    const nonOverlappingHints = localHints.filter((hint) => {
        ...
        for (const verticalCoord of verticalCoords) {
            for (const horizontalCoord of horizontalCoords) {
                const elementFromPoint = getElementFromPoint(horizontalCoord, verticalCoord);
    ...
    console.log(nonOverlappingHints);
